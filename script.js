...
function handleMotion(event) {
  ...
  // UPWARD FLICK: Open shop (must be stronger than Z motion to avoid accidental triggers)
  if (Math.abs(accY) > 9 && Math.abs(accY) > Math.abs(accZ) * 1.5) {
...
function spawnHitEffect(clientX, clientY) {
  ...
  text.innerText = sounds[Math.floor(Math.random() * sounds.length)];
...
function animate() {
  ...
  if (isPunching && activeGlove) {
    ...
    if (punchProgress <= 1.0) {
      let t =
        punchProgress < 0.4
          ? Math.sin((punchProgress / 0.4) * (Math.PI / 2))
          : 1 - Math.pow((punchProgress - 0.4) / 0.6, 2);
